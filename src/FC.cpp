...
static smartPtr < uint8_t > fcBuf;	// for safe unsigned access
static smartPtr < int8_t > fcBufS;	// for safe signed access
...
static const uint8_t FC_silent_data[8] = {
...
// Index is AND 0x7f.
static const uint16_t FC_periods[(5 + 6) * 12 + 4] = {
...
bool FC_init(void *data, size_t len, int start, int end)
{
	...
	fcBufS.setBuffer((int8_t *) data, len);
	fcBuf.setBuffer((uint8_t *) data, len);
	...
	FC_admin.offsets.FC_silent = len - sizeof(FC_silent_data);
	...
	dumpLines(fcBuf, FC_admin.offsets.trackTable,
		  be32toh(*(uint32_t*)&fcBuf[4]), TRACKTAB_ENTRY_LENGTH);
	cout << endl;
#endif

	// At +8 is offset to pattern data.
	FC_admin.offsets.patterns = be32toh(*(uint32_t*)&fcBuf[8]);
	// At +12 is length of patterns.
	// Divide by pattern length to get number of used patterns.
	// The editor is limited to 128 patterns.
	FC_admin.usedPatterns = be32toh(*(uint32_t*)&fcBuf[12]) / PATTERN_LENGTH;
#if defined(DEBUG)
	...
	FC_admin.offsets.sndModSeqs = be32toh(*(uint32_t*)&fcBuf[16]);
	...
	FC_admin.usedSndModSeqs = be32toh(*(uint32_t*)&fcBuf[20]) / 64;
#if defined(DEBUG)
	...
	FC_admin.offsets.volModSeqs = be32toh(*(uint32_t*)&fcBuf[24]);
	// At +28 is total length of volume modulation sequences.
	// Divide by sequence length to get number of used sequences.
	// Each sequence is 64 bytes long.
	FC_admin.usedVolModSeqs = be32toh(*(uint32_t*)&fcBuf[28]) / 64;
#if defined(DEBUG)
	...
	uint32_t sampleOffset = be32toh(*(uint32_t*)&fcBuf[32]);
	uint32_t sampleHeader = FC14_SMPHEADERS_OFFSET;
	...
	for (int sam = 0; sam < 10; sam++) {
		...
		uint16_t sampleLength = be16toh(*(uint16_t*)&fcBuf[sampleHeader]);
		FC_SOUNDinfo.snd[sam].len = sampleLength;
		FC_SOUNDinfo.snd[sam].repOffs = be16toh(*(uint16_t*)&fcBuf[sampleHeader + 2]);
		FC_SOUNDinfo.snd[sam].repLen = be16toh(*(uint16_t*)&fcBuf[sampleHeader + 4]);
...
#if defined(DEBUG)
		cout << dec << setw(6) << (intptr_t) (FC_SOUNDinfo.snd[sam].start - fcBuf.tellBegin()) << " "
		     << dec << setw(4) << (intptr_t) (FC_SOUNDinfo.snd[sam].len * 2L) << " "
		     << dec << setw(6) << (intptr_t) (FC_SOUNDinfo.snd[sam].repOffs) << " "
		     << dec << setw(6) << (intptr_t) (FC_SOUNDinfo.snd[sam].repLen * 2L) << " " << endl;
		dumpLines(fcBuf, (uintptr_t) (FC_SOUNDinfo.snd[sam].start - fcBuf.tellBegin()), 16, 16);
#endif
	}
	...
	if (isSMOD) {
		...
		const uint8_t *wavePtr = SMOD_waveforms;
		int infoIndex = 0;
...
#if defined(DEBUG)
			cout << dec << setw(6) << (intptr_t) (FC_SOUNDinfo.snd[sam].start - wavePtr) << " "
			     << dec << setw(4) << (intptr_t) (FC_SOUNDinfo.snd[sam].len * 2L) << " "
			     << dec << setw(6) << (intptr_t) (FC_SOUNDinfo.snd[sam].repOffs) << " "
			     << dec << setw(6) << (intptr_t) (FC_SOUNDinfo.snd[sam].repLen * 2L) << " " << endl;
#endif
	...
	} else			//if (isFC14)
	{
		...
		uint32_t waveOffset = be32toh(*(uint32_t*)&fcBuf[36]);
		// Module offset to array of waveform lengths.
		uint32_t waveHeader = FC14_WAVEHEADERS_OFFSET;
		for (int wave = 0; wave < 80; wave++) {
			int sam = 10 + wave;
			FC_SOUNDinfo.snd[sam].start   = waveOffset + fcBuf.tellBegin();
			uint8_t waveLength              = fcBuf[waveHeader++];
	...
	uint32_t trackTabLen = be32toh(*(uint32_t*)&fcBuf[4]);
#if defined(DEBUG)
	...
	for (int chan = 0; chan < 4; chan++) {
		...
		FC_CHdata[chan].trackEnd          = FC_CHdata[chan].trackStart + (end != 0 ? std::min<uint32_t>(TRACKTAB_ENTRY_LENGTH * end, trackTabLen) : trackTabLen);
		FC_CHdata[chan].trackPos          = TRACKTAB_ENTRY_LENGTH * start;
		...
		uint32_t trackTabPos                = FC_CHdata[chan].trackStart + FC_CHdata[chan].trackPos;
		uint16_t pattern                     = fcBuf[trackTabPos++];	// PT
	...
	//
	uint8_t songSpeed = fcBuf[FC_CHdata[0].trackStart + FC_CHdata[0].trackPos + 12];
	if (songSpeed == 0)
...
void FC_nextNote(_FC_CHdata & FC_CHXdata)
{
	...
	uint32_t pattOffs = FC_CHXdata.pattStart + FC_CHXdata.pattPos;
	...
	if (FC_CHXdata.pattPos == PATTERN_LENGTH || (isFC14 && fcBuf[pattOffs] == PATTERN_BREAK)) {
		...
		uint32_t trackOffs = FC_CHXdata.trackStart + FC_CHXdata.trackPos;
		...
		if (++FC_admin.RScount == 5) {
			FC_admin.RScount = 1;
			uint8_t newSpeed = fcBuf[trackOffs + 12];	// RS (replay speed)
		...
		uint16_t pattern = fcBuf[trackOffs++];	// PT
	...
	if (FC_CHXdata.dmaMask == 1 && FC_CHXdata.pattPos == 0) {
		...
		uint32_t tmp = FC_CHXdata.trackStart + FC_CHXdata.trackPos;
		for (int t = 0; t < 13; ++t)
	...
	uint8_t note = fcBuf[pattOffs++];
	uint8_t info1 = fcBuf[pattOffs];	// info byte #1
	...
	if (note != 0) {
		...
		uint16_t sound = (fcBuf[pattOffs] & 0x3f) + FC_CHXdata.soundTranspose;
		//
		...
		uint32_t seqOffs;	// the modulation sequence for this sound
...
inline void FC_setWave(_FC_CHdata & FC_CHXdata, uint8_t num)
{
...
{
	uint32_t seqOffs = FC_CHXdata.sndSeq + FC_CHXdata.sndSeqPos;
	...
	if (fcBuf[seqOffs] == SNDMOD_END) {
		...
		uint16_t i = fcBuf[seqOffs + 1];	// sample/pack nr.
		if (i < 10)	// sample or waveform?
		{
			uint32_t sndOffs = FC_SOUNDinfo.snd[i].start - fcBuf.tellBegin();
			// "SSMP"? sample-pack?
			if (fcBuf[sndOffs] == 0x53 && fcBuf[sndOffs + 1] == 0x53 &&
				...
				uint32_t smpStart = sndOffs + 320;
				i = fcBuf[seqOffs + 2];	// sample nr.
				i <<= 4;	// *16 (block size)
				sndOffs += i;
				smpStart += be32toh(*(uint32_t*)&fcBuf[sndOffs]);
				FC_CHXdata.pSampleStart = smpStart + fcBuf.tellBegin();
				FC_CHXdata.ch->paula.start = FC_CHXdata.pSampleStart;
				FC_CHXdata.ch->paula.length = be16toh(*(uint16_t*)&fcBuf[sndOffs + 4]);
				FC_CHXdata.ch->takeNextBuf();

				// (FC14 BUG-FIX): Players set period here by accident.
				// m68k code move.l 4(a2),4(a3), but 6(a3) is period.

				FC_CHXdata.repeatOffset = be16toh(*(uint16_t*)&fcBuf[sndOffs + 6]);
				FC_CHXdata.repeatLength = be16toh(*(uint16_t*)&fcBuf[sndOffs + 8]);
...
void FC_processPerVol(_FC_CHdata & FC_CHXdata)
{
	...
	do {
		...
		// Time to set next volume level? NE => no, EQ => yes.
			...
			do {
				...
				uint32_t seqOffs = FC_CHXdata.volSeq + FC_CHXdata.volSeqPos;
				uint8_t command = fcBuf[seqOffs];
	...
	if (FC_CHXdata.vibDelay == 0) {
		uint16_t noteTableOffset = tmp1;	// tmp1 is note*2;

		int16_t vibDelta = FC_CHXdata.vibAmpl;
	...
	if (isSMOD || FC_CHXdata.portDelayFlag != 0) {
		int8_t param = FC_CHXdata.portaInfo;
		if (param != 0) {
	...
	if (FC_CHXdata.pitchBendDelayFlag != 0) {
		if (FC_CHXdata.pitchBendTime != 0) {
			...
			int8_t speed = FC_CHXdata.pitchBendSpeed;
			if (speed != 0) {
